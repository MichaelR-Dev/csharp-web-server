...
public static class Server
{
    ...
    // Handler instances for different HTTP methods
    private static readonly GetHandler get = new GetHandler();
    private static readonly PostHandler post = new PostHandler();
    private static readonly PutHandler put = new PutHandler();
    private static readonly PatchHandler patch = new PatchHandler();
    private static readonly DeleteHandler delete = new DeleteHandler();
    ...
    public static void StartServer(int port = 5050, string localAddr = "127.0.0.1")
    {
        if (IPAddress.TryParse(localAddr, out IPAddress? parsedAddr) && parsedAddr != null && port > 0 && port <= 65535)
        {
            ...
            Console.WriteLine($"Web Server started on {localAddr} on port {port}... Press Q or ^C to Stop...");
